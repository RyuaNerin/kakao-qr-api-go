...
const (
	UserAgent = "Mozilla/5.0 (iPhone; CPU iPhone OS 14_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 KAKAOTALK 9.0.3"
...
var (
	...
	regToken = regexp.MustCompile(`"token":\s*"(.+?)"`)
...
func main() {
	...
	// Init Chromium
	dpContext, cancel := chromedp.NewExecAllocator(
		context.Background(),
...
func generateToken(ctx context.Context, doLogin bool) (string, error) {
	...
	if res.StatusCode != http.StatusOK {
		return "", fmt.Errorf("qr_check_in returned %d %s", res.StatusCode, res.Status)
	}
	...
	matches := regToken.FindSubmatch(data)
	if matches == nil {
	...
	req, _ = http.NewRequest(
		"GET",
		"https://accounts.kakao.com/qr_check_in/request_qr_data.json?lang=ko&os=ios&webview_v=2&is_under_age=false&token="+string(matches[1]),
		nil,
	)
	req.Header = header
	req.WithContext(ctx)
	...
	if res.StatusCode != http.StatusOK {
		return "", fmt.Errorf("qr_check_in/request_qr_data.json returned %d %s", res.StatusCode, res.Status)
	}
